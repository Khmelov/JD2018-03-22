...
public class Test_jd01_02 {
    ...
    public void testTaskBstep3_QEquation__TaskB() throws Exception {
        ...
        System.out.println("для a=2, для b=4, для c=2, ожидается решение x=-1.0");
        m.invoke(null, 2, 4, 2);
        ok.include("-1");
        System.out.println("для a=1, для b=-1, для c=-2, ожидается решение x1=-1.0 x2=2.0 ");
        m.invoke(null, 1, -1, -2);
        ok.include("-1").include("2");
        System.out.println("для a=2, для b=4, для c=4, ожидается решение корней нет");
        m.invoke(null, 2, 4, 4);
