package by.it.kovko.jd01_15;

import java.io.*;
import java.util.Random;

public class TaskA {
    public static void main(String[] args) {

        int matrix[][]= new int [6][4];
        String fout = getPath(TaskA.class, "matrix.txt");
        try (PrintWriter printer = new PrintWriter(new FileWriter(fout))){
            for(int i=0;i<6;++i) {
                for (int j = 0; j < 4; ++j)
                    printer.printf("%3d ", (int) (Math.random() * 30) - 14);
                printer.println();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }


        try (BufferedReader reader = new BufferedReader(new FileReader(fout))){
            for (int i = 0; i < matrix.length; i++) {
                String[] elements =reader.readLine().trim().split("\\s+");
                for (int j = 0; j < matrix[i].length; j++) {
                    matrix[i][j]=Integer.parseInt(elements[j]);
                    System.out.printf("%3d ", matrix[i][j]);
                }
                System.out.println();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    }


    static String getPath(Class<?> cl, String filename) {
        return getPath(cl)+filename;
    }
    static String getPath(Class<?> cl) {
        String path = System.getProperty("user.dir");
        path+=(File.separator+"src"+File.separator);
        path+=cl.getName().replace(cl.getSimpleName(),"").replace(".",File.separator);
        return path;
    }
}
